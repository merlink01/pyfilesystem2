...
from .dokanerrors import *
# ~ from fs.errors import *
from .dokanpath import *
# ~ from fs.path import *
...
class FSOperations(object):
    ...
    def GetFileInformation(self, path, buffer, info):
        path = self._dokanpath2pyfs(path)
        finfo = self.fs.getinfo(path,namespaces=['basic','details'])
    ...
    def FindFilesWithPattern(self, path, pattern, fillFindData, info):
        ...
        # ~ for (nm, finfo) in self.fs.scandir(path):
    ...
    def DeleteDirectory(self, path, info):
        path = self._dokanpath2pyfs(path)
        for nm in self.fs.listdir(path):
            if not self._is_pending_delete(combine(path, nm)):
    ...
    def GetDiskFreeSpace(self, nBytesAvail, nBytesTotal, nBytesFree, info):
        ...
        try:
            ...
            nBytesTotal[0] = large_amount
            nBytesAvail[0] = large_amount
        return STATUS_SUCCESS
    ...
    def _info2finddataw(self, path, info, data=None, hinfo=None):
        ...
        data.ftCreationTime = _datetime2filetime(info.get('details',"created", None))
        data.ftLastAccessTime = _datetime2filetime(info.get('details',"accessed", None))
        data.ftLastWriteTime = _datetime2filetime(info.get('details',"modified", None))
        data.nFileSizeHigh = info.get('details',"size", 0) >> 32
        data.nFileSizeLow = info.get('details',"size", 0) & 0xffffffff
