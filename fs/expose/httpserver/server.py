...
class PyfsFileServerHandler(http.server.BaseHTTPRequestHandler):
    ...
    server_version = "Pyfs2FileServer/" + __version__
    
    def __init__(self, *args,**kwargs):
        assert hasattr(self,'filesystem')
        super(PyfsFileServerHandler, self).__init__(*args,**kwargs)
    ...
    def deal_post_data(self):
        ...
        try:
            out = self.filesystem.open(fn, 'wb')
        except FileExpected:
    ...
    def send_head(self):
        ...
        if self.filesystem.isdir(path):
            ...
            # ~ for index in "index.html", "index.htm":
                ...
                # ~ if self.filesystem.exists(index):
                    # ~ path = index
                    # ~ break
            else:
        ...
        try:
            ...
            f = self.filesystem.open(path, 'rb')
        except ResourceNotFound:
            self.send_error(404, "File not found")
            return None
        self.send_response(200)
        self.send_header("Content-type", ctype)
        self.send_header("Content-Length", self.filesystem.getsize(path))
        #todof
    ...
    def list_directory(self, path):
        ...
        try:
            list = self.filesystem.listdir(path)
        ...
        for name in list:
            ...
            if self.filesystem.isdir(fullname):
                displayname = name + "/"
                linkname = name + "/"
            if self.filesystem.islink(fullname):
                displayname = name + "@"
...
def serve(filesystem, host='127.0.0.1', port=8000):
        PyfsFileServerHandler.filesystem = filesystem

        # Port 0 means to select an arbitrary unused port
        server = ThreadingSimpleServer((host, port), PyfsFileServerHandler)
        
        print('Serving Filesystem: %s'%repr(filesystem))
        print('Started Server on %s Port %s'%server.server_address)
